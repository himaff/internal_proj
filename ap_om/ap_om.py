...
class ap_om(osv.osv):
    ...
    _columns = {
        ...
        'route_to_go': fields.char('route to go', size=128, required=True),
        'route_return': fields.char('route of return', size=128, required=True),
        'date_hour_depart': fields.date('date hour depart', required=True),
        'date_hour_return': fields.date('date hour return', required=True),
        'accompanying_person': fields.many2many('hr.employee', 'fleet_vehicle_vehicle_tag_rel', 'vehicle_tag_id','tag_id', 'accompanying person', copy=False),
