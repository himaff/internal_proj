...
class reparation_fleet(osv.osv):
    _name ='fleet.vehicle.log.services'
    _inherit ='fleet.vehicle.log.services'
	
    _columns={
        'unit': fields.char('Unit', size=128, required=True),
        'quantity': fields.float('Quantity', digits=(2,1)),
        'unit_price': fields.float('Unit Price', digits=(2,1)),
		'total_amount': fields.float('Total amount', digits=(2,1)),
		        		
    }		

    def calcul_total_amount(self, cr, uid, ids, calcul, context=None):
        unit_p=calcul+0
        tot=calcul*unit_p
        valeur={'unit_price':unit_p,'total_amount':tot}
        return {"value":valeur}
